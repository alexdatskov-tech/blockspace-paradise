  ...
  head: () => ({
    meta: [
      { title: "System Status — BlockForge" },
      {
        name: "description",
        content:
          "View the current operational status of BlockForge hosting, networking, control panel, and backups.",
      },
      { property: "og:title", content: "BlockForge System Status" },
      {
        property: "og:description",
        content: "Current availability for BlockForge hosting services and infrastructure.",
      },
    ...
    <main className="min-h-screen bg-background px-4 pb-16 pt-6 text-foreground">
      ...
      <section className="mx-auto max-w-5xl pt-20 sm:pt-28">
        <div className="mb-12 max-w-3xl animate-rise-soft">
          <span className="inline-flex items-center gap-2 rounded-full border border-gold/30 bg-gold/10 px-3 py-1.5 text-xs font-bold uppercase text-gold">
            Beta status page
          </span>
          <h1 className="mt-6 text-balance text-4xl font-bold sm:text-6xl">
            Every block is online.
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-muted-foreground">
            Live service reporting is currently in beta. All BlockForge systems are operating
            normally.
          </p>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-4 rounded-xl border border-primary/40 bg-primary/10 p-6 shadow-[0_0_40px_color-mix(in_oklab,var(--primary)_12%,transparent)] sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold">Systems operational</h2>
          ...
          <span className="flex items-center gap-2 text-xs font-semibold text-primary">
            <span className="animate-status size-2 rounded-full bg-primary" /> Updated moments ago
          </span>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card">
          {services.map(({ icon: Icon, name, detail, uptime }) => (
            <div
              key={name}
              className="grid gap-4 border-b border-border p-5 last:border-b-0 sm:grid-cols-[1fr_auto] sm:items-center sm:px-6"
            >
              <div className="flex items-center gap-4">
              ...
              <div className="flex items-center justify-between gap-6 pl-14 sm:justify-end sm:pl-0">
        ...
        <div className="mt-6 flex items-center gap-3 rounded-lg border border-border bg-secondary/35 p-4 text-xs text-muted-foreground">
          <ShieldCheck className="size-4 shrink-0 text-primary" />
          <span>
            Monitoring checks run continuously across hosting, network, panel, and backup services.
          </span>
          <Clock3 className="ml-auto hidden size-4 shrink-0 sm:block" />
        </div>
      </section>
    </main>
  );
